...
<input type="hidden" id="ti-noreg-version" value="10.5" />
...
<option value="<?php echo esc_attr($type); ?>" <?php echo $type == $review_text_mode ? 'selected' : ''; ?>><?php echo TrustindexPlugin_google::___($translated); ?></option>
<?php endforeach; ?>
</select>
</form>
</div>
...
<?php endif; ?>
<?php if(in_array($style_id, [ 4, 6, 7, 15, 16, 19, 31, 33, 36, 37, 38, 39, 44 ])): ?>
<span class="ti-checkbox row">
...
<?php if($widget_has_reviews): ?>
<span class="ti-checkbox row">
<input type="checkbox" name="footer-filter-text" value="1" <?php if($footer_filter_text): ?>checked<?php endif;?>>
<label><?php echo TrustindexPlugin_google::___("Show minimum review filter condition"); ?></label>
</span>
<?php endif; ?>
<?php if($widget_has_reviews && in_array(ucfirst($trustindex_pm_google->shortname), TrustindexPlugin_google::$verified_platforms)): ?>
<span class="ti-checkbox row">
