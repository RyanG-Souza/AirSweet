...
$chosed_platform = new self($force_platform, $file_path, "do-not-care-10.5", "do-not-care-Widgets for Google Reviews", "do-not-care-Google");
...
if($version >= 10.1)
{
if(count($wpdb->get_results('SHOW COLUMNS FROM `'. $table_name .'` LIKE "reply"')) == 0)
{
$wpdb->query('ALTER TABLE `'. $table_name .'` ADD reply TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL AFTER date');
if(count($wpdb->get_results('SHOW COLUMNS FROM `'. $table_name .'` LIKE "replied"')) == 1)
{
$wpdb->query('ALTER TABLE `'. $table_name .'` DROP replied');
}
...
if(count($wpdb->get_results('SHOW COLUMNS FROM `'. $table_name .'` LIKE "reviewId"')) == 0)
{
$wpdb->query('ALTER TABLE `'. $table_name .'` ADD reviewId TEXT NULL AFTER date');
}
}
}
...
$footer_filter_text = get_option($this->get_option_name('footer-filter-text'), 0);
$need_to_parse = true;
if($only_preview)
{
$content = false;
...
'footer_filter_text' => $footer_filter_text
]);
...
{
$widget_type = self::$widget_templates[ 'templates' ][ $array['style_id'] ]['type'];
...
if(function_exists('mb_strtolower'))
{
$rating_text_ucfirst = mb_substr($rating_text, 0, 1, 'UTF-8') . mb_strtolower(mb_substr($rating_text, 1, null, 'UTF-8'));
}
...
$rating_text,
$rating_text_ucfirst,
isset($array['page_details']['avatar_url']) ? $array['page_details']['avatar_url'] : null,
...
if($array['no_rating_text'] && in_array($array['style_id'], [ 4, 6, 7, 15, 16, 19, 31, 33, 36, 37, 38, 39, 44 ]))
{
...
if($array['footer_filter_text'] && (!in_array($widget_type, [ 'button', 'badge', 'floating' ]) || in_array($array['style_id'], [ 23, 30, 32, 53 ])))
{
$filter_text = $this->get_footer_filter_text($array['language']);
if(!$array['no_rating_text'] && !in_array($array['style_id'], [ 5, 8, 9, 10, 13, 18, 23, 30, 31, 32, 33, 34, 53, 54 ]))
{
...
$array['content'] = str_replace('<!-- FOOTER FILTER TEXT -->', function_exists('mb_strtolower') ? mb_strtolower($filter_text) : strtolower($filter_text), $array['content']);
}
else
{
$array['content'] = str_replace('<!-- FOOTER FILTER TEXT -->', $filter_text, $array['content']);
}
}
else
{
...
{
$filter = get_option($this->get_option_name('filter'), $this->get_widget_default_filter());
$lang_exists = self::$widget_footer_filter_texts && isset(self::$widget_footer_filter_texts[ $lang ]);
$text = $lang_exists ? self::$widget_footer_filter_texts[ $lang ]['latest'] : 'Showing our latest reviews';
...
$text = str_replace('RATING_STAR_FILTER', $replace, $lang_exists ? self::$widget_footer_filter_texts[ $lang ]['star'] : 'Showing only RATING_STAR_FILTER star reviews');
}
return $text;
}
